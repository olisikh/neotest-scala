...
- **AsyncWordSpec / AsyncFunSpec / AsyncFeatureSpec families**
  - Async variants for unsupported DSLs remain unsupported with the base style itself.
...
- TextSpec fallback to mutable parsing when `s2"""` exists but no `$ref` entries are found
...
- **`!` operator non-block forms**
  - Expression forms like `"name" ! ok` (without block-style body) still need dedicated parsing/matching work.

### Complexity Assessment

- **Medium**: improve immutable/fragment parsing coverage without regressing existing mutable/TextSpec behavior.
...
1. **ScalaTest: implement AnyFunSpec / AnyWordSpec / AnyFeatureSpec** (bigger DSL work + likely path-building).
2. **specs2: expand fragment-heavy immutable forms** (parser work; higher risk).
