...
1. **ScalaTest: add Async*/Fixture* detection** (reuses existing queries).
2. **ScalaTest: implement AnyPropSpec** (new but fairly localized query).
3. **ScalaTest: implement AnyFunSpec / AnyWordSpec / AnyFeatureSpec** (bigger DSL work + likely path-building).
4. **specs2: expand fragment-heavy immutable forms** (parser work; higher risk).
