...
- `test("name") { ... }` and interpolated forms like `test(s"...")` (including nested `test("outer") { test("inner") { ... } }`)

### Common Missing Shapes

- **Non-literal test names**:
  - `test(nameVar) { ... }` and other runtime-computed identifiers remain hard to resolve statically.
  - For interpolated names, discovery/matching works, but single-test execution may need to run at suite scope when selector cannot be made exact.
