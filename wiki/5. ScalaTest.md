...
WordSpec style is supported, including `AsyncWordSpec`:
...
FunSpec style is supported, including `AsyncFunSpec` and fixture variants:
...
class MyFunSpec extends AnyFunSpec {
  describe("List operations") {
    it("supports checks") { assert(List(1).head == 1) }
  }
}
```
...
FeatureSpec style is supported, including async and fixture variants:
...
class MyFeatureSpec extends AnyFeatureSpec {
  Feature("Authentication") {
    Scenario("successful login") { assert(true) }
  }
}
```
...
RefSpec style is supported via zero-argument method discovery:
...
| `"context" should { "test" in { } }` | ✅ | WordSpec style |
| `describe("context") { it("test") { } }` | ✅ | FunSpec style |
| `Feature("X") { Scenario("Y") { } }` | ✅ | FeatureSpec style |
| ``def `name`(): Unit = { }`` | ✅ | RefSpec style |
