...
For these interpolated/suite-scope runs, utest JUnit output commonly reports numeric test ids (`0`, `1`, `2`, ...). neotest-scala maps those numeric ids back to discovered test order so failures are attributed to the correct test.
...
When the name is interpolated (dynamic), neotest-scala runs the suite path and resolves the selected test from result mapping.
...
> For interpolated names, the effective execution path is `package.ObjectName` (suite scope).
